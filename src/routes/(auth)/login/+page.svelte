    ...
    {:else}
        <div transition:blur={{ duration: 200, amount: 5, easing: cubicOut }} class="otp-content">
            <form id="validate-otp" class="validate-otp-form" {...validateOTP}>
                ...
                <input {...validateOTP.fields.totp.as('text')} id="one-time-code" autocomplete="one-time-code" pattern="^[0-9]?[0-9]?[0-9]?[0-9]?[0-9]?[0-9]?$" bind:value={totp} style="opacity: 0;position:absolute;pointer-events:none;">
                <div class="otp-hint">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="otp-buttons">
                ...
                <button title="" form="validate-otp" type="submit" class="theme-button blue-hover {totp.length === 6 ? '' : 'disabled'}">
                    Verify & Continue
                </button>
            </div>
        </div>
